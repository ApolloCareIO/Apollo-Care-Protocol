...
impl FraudFlags {
    ...
    pub fn count(&self) -> u8 {
        ...
        if self.frequency_anomaly {
            count += 1;
        }
        if self.document_issues {
            count += 1;
        }
        if self.provider_flagged {
            count += 1;
        }
        if self.service_mismatch {
            count += 1;
        }
        if self.timing_anomaly {
            count += 1;
        }
        if self.geographic_anomaly {
            count += 1;
        }
        count
    }
...
impl ClaimsConfig {
    ...
    pub fn get_effective_shock_threshold(member_count: u32, total_reserves: Option<u64>) -> u64 {
...
mod tests {
    ...
    fn test_attestation_recommendation_equality() {
        assert_eq!(
            AttestationRecommendation::ApproveFull,
            AttestationRecommendation::ApproveFull
        );
        assert_ne!(
            AttestationRecommendation::ApproveFull,
            AttestationRecommendation::Deny
        );
    }
}
