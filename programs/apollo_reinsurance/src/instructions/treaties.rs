...
use crate::state::{ReinsuranceConfig, ReinsuranceTreaty, ReinsuranceLayerType, TreatyStatus};
...
pub fn create_treaty(
    ...
    config.total_treaties = config.total_treaties.checked_add(1)
    ...
    msg!("Treaty {} created: {:?}", treaty.treaty_id, params.layer_type);
...
pub fn activate_treaty(ctx: Context<ActivateTreaty>) -> Result<()> {
    ...
    config.active_treaties = config.active_treaties.checked_add(1)
        .ok_or(ReinsuranceError::Overflow)?;
...
pub fn pay_premium(
    ...
    // Check budget
    let new_ytd = config.premium_paid_ytd.checked_add(amount)
        .ok_or(ReinsuranceError::Overflow)?;
    ...
    // Update tracking
    treaty.premium_paid = treaty.premium_paid.checked_add(amount)
        .ok_or(ReinsuranceError::Overflow)?;
    treaty.last_updated = clock.unix_timestamp;
    ...
    msg!("Premium paid: {} USDC, total: {} of {} USDC",
        amount / 1_000_000,
...
pub fn update_treaty_status(
    ...
    msg!("Treaty {} status: {:?} -> {:?}", treaty.treaty_id, old_status, new_status);
...
pub fn update_treaty_params(
    ...
    if let Some(coinsurance) = params.coinsurance_rate_bps {
        require!(coinsurance <= 10_000, ReinsuranceError::InvalidCoinsuranceRate);
